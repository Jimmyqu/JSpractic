        ...
        <el-form :model="ruleForm" :rules="rules" label-position="top" ref="ruleForm" label-width="100px">
            <el-collapse v-model="activeNames">
                ...
                <el-collapse-item title="合同明细" name="5">
                    ...
                    <el-table class="left" border :data="ruleForm.contractDetailList" style="width: 100%">
                        <el-table-column type="index" label="序号" width="50"></el-table-column>
                        <el-table-column label="结算组织" min-width="150" label-class-name="required">
                            <template slot-scope="{row,$index}" >
                                <el-form-item  :prop="'contractDetailList.' + $index + '.settlementOrganizationId'"
                        ...
                        <el-table-column prop="useCarStartDate" label="用车开始时间" min-width="200" label-class-name="required">
                            <template slot-scope="{row,$index}">
                                <el-form-item :prop="'contractDetailList.' + $index + '.useCarStartDate'"  :rules="rules.required('请选择用车开始时间')">
                        ...
                        <el-table-column prop="useCarEndDate" label="用车结束时间" min-width="200" label-class-name="required">
                            <template slot-scope="{row,$index}">
                                <el-form-item :prop="'contractDetailList.' + $index + '.useCarEndDate'"  :rules="rules.required('请选择用车结束时间')">
                        ...
                        <el-table-column prop="vehicleQty" label="车辆数（台）" min-width="150" label-class-name="required">
                            <template slot-scope="{row,$index}">
                                <el-form-item :prop="'contractDetailList.' + $index + '.vehicleQuantity'" :rules="[{ validator: formRule.standardSize,message: '请输入整数', trigger: 'change' },{required: true, message: '请输入车辆数', trigger: 'change'}]" >
                        ...
                        <el-table-column label="车辆每月费用(元)" min-width="150" label-class-name="required">
                            <template slot-scope="{row,$index}">
                                <el-form-item :prop="'contractDetailList.' + $index + '.vehicleCost'"
                        ...
                        <el-table-column prop="date" label="司机人数（名）" min-width="150" label-class-name="required">
                            <template slot-scope="{row,$index}">
                                <el-form-item :prop="'contractDetailList.' + $index + '.driverQuantity'" :rules="[{ validator: formRule.standardSize,message: '请输入整数', trigger: 'change' },{required: true, message: '请输入司机人数', trigger: 'change'}]">
                        ...
                        <el-table-column label="司机每月费用(元)" min-width="150" label-class-name="required">
                            <template slot-scope="{row,$index}">
                                <el-form-item :prop="'contractDetailList.' + $index + '.driverCost'"
    ...
    export default {
        ...
        methods: {
            ...
            countVehicleQuantity(){
                let sum = 0;
                let data = this.ruleForm.contractDetailList;
                if(data && data.length) {
                    data.forEach( item =>{
                ...
                ajax.get("core/serviceHostingContract/detail/" + id).then(result =>{
                    if (result.status == 0) {
                        ...
                        if( result.data.contractDetailList){
                            ...
                            this.ruleForm.contractDetailList =  result.data.contractDetailList;
                        }
                    } else {
            ...
            addProjectContractDetail() {
                this.ruleForm.contractDetailList.push({
                    settlementOrganizationId:[],
                    useCarStartDate:'',
                    useCarEndDate:'',
                    vehicleQuantity:'',
            ...
            convertData(params){
                ...
                params.contractDetailList.forEach( item =>{
            ...
            deleteItem(index) {
                if (this.ruleForm.contractDetailList.length == 1) {
                    this.$message.error('至少保留一条合同明细');
                    return;
                }
                this.ruleForm.contractDetailList.splice(index, 1);
            }
        },
