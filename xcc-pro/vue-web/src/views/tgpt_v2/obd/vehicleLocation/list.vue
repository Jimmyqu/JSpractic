<template>
    <div class="app-container white-bg list-panel position" v-cloak>
        ...
        <div class="frame" :class="{'hidden':shrink}">
            <div class="radio-group">
                <!--地图头部信息-->
                <el-radio-group v-model="chooseStatus" @change="chooseListByStatus()" v-if="this.activeName==1">
                ...
                <el-radio-group v-model="chooseStatus" @change="chooseListByStatus()" v-if="this.activeName==2">
                    <el-radio label="全部" >全部{{listVehicle.length ? listVehicle.length: 0}}</el-radio>
                    <el-radio label="运行" >在线{{onLineList.length ? onLineList.length: 0}}</el-radio>
                    <el-radio label="停止" >休眠{{sleepList.length ? sleepList.length: 0}}</el-radio>
                    <el-radio label="离线" >故障{{errList.length ? errList.length: 0}}</el-radio>
                </el-radio-group>
                <el-checkbox v-model="showPlate" @change="showPlateDo">显示车牌</el-checkbox>
            </div>
            ...
            <div class="table-box frame-table-box" v-if="selectedList.length > 0">
                ...
                <el-table v-loading="listLoading" :data="selectedList" style="width: 100%;cursor: pointer;" :height="'calc(100vh - 450px)'" ref="table" @cell-click="handleSelectionChange" >
            ...
            <div class="shrink">
                <span @click="shrink = !shrink"><SvgIcon :class="shrink?'open':'retract'" :iconClass="shrink?'open':'retract'"/><el-button type="text">{{shrink?'展开':'收起'}}</el-button></span>
            </div>
        </div>
    ...
    export default {
        ...
        methods: {
            ...
            groupListWireless(){
                ...
                this.listVehicle.forEach(item => {
                    if(item.vehicleStatus == 1){
                        ...
                        item.statusText = `休眠${item.runtime ? item.runtime: ''}`;
                        this.sleepList.push(item);
                    }else if(item.vehicleStatus ==3){
                        item.statusText = `故障${item.runtime ? item.runtime: ''}`;
                        this.errList.push(item);
                    }else{
                        item.statusText = `未定位`;
                    }
            ...
            batchLocation() {
                ...
                ajax.get(`obd/obdVehicleStatus/findVehicleListLocation?vehicleId=${this.vehicleId == null ?'':this.vehicleId}&orgId=${this.originateDeptId}&equType=${this.activeName}`).then(rs => {
                    ...
                    if(this.activeName==1){
                        ...
                        this.groupListWireless(rs.data);
                    }
                    /*选中数据*/
                    this.addListByType();
                    /*标注*/
                    ...
                    if(this.selectedList.length == 1){
                        this.handleSelectionChange(this.selectedList[0])
                    }else{
                        /*聚焦*/
                        this.addPoint(this.selectedList[0]);
                    }
            ...
            handleSelectionChange(item){
                if(item.corlat > 1 && item.corlong > 1){
            ...
            markInfo(currentVehicleTrackList , flag){
                this.ss(currentVehicleTrackList,flag)
            },
            ss(markerArr,flag){//markerArr数据库数据查询的结果
            ...
            openInfoWindow(currentVehicleTrack){
                if(this.activeName==1){
                        ...
                        if(locationDetail.status == 0){
                            ...
                            geoc.getLocation(pt, rs => {
                               // locationDetail.data.localtion = rs.address;
                                ...
                                content += `<p>总里程：${locationDetail.data.totalMileage?(locationDetail.data.totalMileage*1/1000).toFixed(2):0} km <span style="float:right; _position:relative;">运行时长：${locationDetail.data.runTime}</p>`;
                                content += "<p>开始时间：" + locationDetail.data.startTime + "</p>";
                                ...
                                content += `<div class="drop-down-menu1" id="drop-down-menu1" >
                                                    <ul class="el-dropdown-menu "
                                                        ...
                                                        <li tabindex="-1" onclick="vlToLocationRealtime('${locationDetail.data.vehicleId}', '${locationDetail.data.plate}')" class="el-dropdown-menu__item">实时追踪</li>
                                                        <li tabindex="-1" onclick="vlToVehicleLocationTrack('${locationDetail.data.vehicleId}', '${locationDetail.data.plate}','${this.activeName}' )"  class="el-dropdown-menu__item">车辆轨迹</li>
                                                        <li tabindex="-1" onclick="vlToVehicleTrackRecord('${locationDetail.data.plate}')" class="el-dropdown-menu__item">行程记录</li>
                                                    </ul>
                                                 </div>`
                                ...
                                content += `<div class="drop-down-menu2" id="drop-down-menu2" >
                                                    <ul class="el-dropdown-menu el-popper"
                                                        style="transform-origin: center top 0px; ">
                                                        <li tabindex="-1" onclick="vlShowTimeFence('${locationDetail.data.vehicleId}')" class="el-dropdown-menu__item">时间栅栏</li>
                                                        <li tabindex="-1" onclick="vlElectronFence('${locationDetail.data.vehicleId}')"  class="el-dropdown-menu__item">电子围栏</li>
                                                        <li tabindex="-1" onclick="vlAreaFence('${locationDetail.data.vehicleId}')" class="el-dropdown-menu__item">区域栅栏</li>
                                                    </ul>
                                                 </div>`
                                content += `</div>`
                    ...
                    if(currentVehicleTrack.corlong < 1 && currentVehicleTrack.corlat < 1){
                        this.$message.warning("未获取到车辆定位信息！");
                        return;
                    }
                    /*根据经纬度获取车辆定位地址*/
