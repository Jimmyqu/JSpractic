                            ...
                            <template slot-scope="{row,$index}" >
                                <el-form-item  :prop="'contractExecutionDetailList.' + $index + '.settlementOrganizationId'"
                                    ...
                                    <tree-select v-model="row.settlementOrganizationId" placeholder="请选择组织" type="one"
                                                 url="admin/organization/companyTree" :disabled="type=='edit' && row.isNew==0"></tree-select>
                                </el-form-item>
                            </template>
                        </el-table-column>
                            ...
                            <template slot-scope="{row,$index}">
                                <el-form-item :prop="'contractExecutionDetailList.' + $index + '.useCarStartDate'"  :rules="rules.required('请选择用车开始时间')">
                                    <el-date-picker type="date" placeholder="请选择" v-model="row.useCarStartDate"
                                                    value-format="yyyy-MM-dd" :editable="false" :disabled="type=='edit' && row.isNew==0"></el-date-picker>
                                </el-form-item>
                            </template>
                        </el-table-column>
    ...
    export default {
        ...
        methods: {
            ...
            getFormData: function (id) {
                this.fileList = [];
                ajax.get("core/serviceHostingContractExecution/detail/" + id).then(result =>{
                    if (result.status == 0) {
                        if( result.data){
                    ...
                    if (valid) {
                       ...
                       // params.originateDeptId = params.originateDeptId.join();
                        ...
                        if(this.type == 'perform')
                            url="/core/serviceHostingContractExecution/perform";
                        else if(this.type == 'edit')
                            url="/core/serviceHostingContractExecution/edit";
