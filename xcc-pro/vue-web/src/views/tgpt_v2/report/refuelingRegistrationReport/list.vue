...
<script>
    ...
    export default {
        name: 'refuelingRegistrationReport',
        mixins: [tool],
        ...
        data() {
            return {
                ...
                mChart1:null,
                mChart2:null,
                listUrl: 'report/refuelingRegistrationReport/byVehicleList',
                excelUrl: 'report/refuelingRegistrationReport/excel',
                showExportExcelBtn: this.getCurrentUserAuthority('refuelingRegistrationReport/exportExcel'),
                showSearch: false,
                carUseMonth: [],
                originateDeptId: [],
                oilCapacity: 0,
        ...
        methods: {
            ...
            handleClick(){
                if(this.activeName == "2"){
                    this.listUrl = 'report/refuelingRegistrationReport/byServiceList';
                }else {
                    this.listUrl = 'report/refuelingRegistrationReport/byVehicleList';
                }
                this.getList();
            },
            toDetail(plate, regionId){
                ...
                var url = "";
                if(plate){
                    url = "/tgpt_v2/traffic/refuelingRegistration?startTime="+startTime+"&endTime="+endTime+"&plate="+plate;
                }
                if(regionId){
                    url = "/tgpt_v2/traffic/refuelingRegistration?startTime="+startTime+"&endTime="+endTime+"&regionId="+regionId;
                }
            ...
            exportExcel(){
                if(this.activeName == "2"){
                    this.excelUrl = 'report/refuelingRegistrationReport/excelService?';
                }else {
                    this.excelUrl = 'report/refuelingRegistrationReport/excel?';
                }
            ...
            getFuelConsumption() {
                ...
                ajax.get('report/refuelingRegistrationReport/getChartData', $this.searchParam).then(res => {
                    if(res.status == 0){
                        let data = {
                            date:[],
                            amount:[],
                        ...
                        for (let i=0;i < res.data.length;i++) {
                            ...
                            oilCapacity += parseFloat(item.oilCapacity);
                            oilCost += parseFloat(item.oilCost);
                        }
                        this.oilCapacity = oilCapacity.toFixed(2);
