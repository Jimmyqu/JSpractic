...
import { formatTimeDuration, formatMoney, formatColorWrapper, decodeMoney, formatModel } from '@/utils/format';
...
class AnalysisBusinessSummaryServiceUser extends Component {
  state = {
      ...
      children: [
        ...
        },
        {
          ...
          render: this.genLinkWrapperRender(
            ...
            (value, record) => {
              const { salesId, professionalId } = record;
              return {
                salesIds: salesId, // key是多个
                professionalId, // 单个
                careerId: value, // 单个
      ...
      children: [
        {
          ...
          render: this.genLinkWrapperRender(
            ...
            (value, record) => {
              const { salesId, professionalId, careerId, platformUserId } = record;
              const {
              ...
              return {
                ...
                professionalId, // 单个
                careerId, // 单个
                platformUserIds: platformUserId, // key是多个,
          ...
          <Card bordered={false} loading={currentItem == null}>
            <Tabs defaultActiveKey={tabKey} activeKey={tabKey} onChange={this.onTabChange}>
              <TabPane tab="现金流水" key="financeFlow">
                <Datatable
                  tableId="financeFlow"
                  url="/analysis/serviceUser/summary.do?queryType=0"
                  pagination={false}
                  bodyScroll={false}
                  dataSourceRender={this.dataSourceRender}
                  columns={this.columns}
                  rowKey={record => `${record.salesId}-${record.sysUserId}-${record.careerId}`}
              ...
              <TabPane tab="消费收入" key="consume">
                <Datatable
                  tableId="consume"
                  url="/analysis/serviceUser/summary.do?queryType=1"
                  pagination={false}
                  bodyScroll={false}
                  dataSourceRender={this.dataSourceRender}
                  columns={this.columns}
...
export default AnalysisBusinessSummaryServiceUser;
