            ...
            <el-table :ref="getRefName" :max-height="tableHeight" v-loading="listLoading" border style="width: 100%" :data="list">
                <el-table-column fixed="left" label="操作" width="150">
                    <template slot-scope="scope">
                        ...
                        <el-button
                            v-show="showVersionsBtn && scope.row.versionList"
                            @click="showVersionList(scope.row)" type="text" size="small">
                            版本号
                        </el-button>
                        <el-button
                            ...
                            @click="editContractExecution(scope.row)" type="text" size="small">
                            编辑
                        </el-button>
                    </template>
                </el-table-column>
...
<script>
    import VersionListPanel from '@/views/tgpt/project/serviceHosting/contractExecution/versionList'
    ...
    export default {
        ...
        mixins: [tool],
        components: {TreeSelect,VersionListPanel },
        data(){
            return {
                listUrl: "/core/serviceHostingContractExecution/list",
                transmissionList:[],
                commitDisabled:false,
                formData: {},
                modelForm:{},
                organization:[],
                showPerformBtn:this.getCurrentUserAuthority("serviceHostingContractExecution/perform"),
                showEditBtn:this.getCurrentUserAuthority("serviceHostingContractExecution/edit"),
                showExportBtn:this.getCurrentUserAuthority("serviceHostingContractExecution/export"),
                showVersionsBtn:this.getCurrentUserAuthority("serviceHostingContractExecution/versionList"),
                showSubmitApprovalBtn:this.getCurrentUserAuthority("serviceHostingContractExecution/commit"),
                showSearch: false,
                contractStatus:[],
                branchOffice: [],
                contractStartDate:"",
        ...
        methods: {
            ...
            submitApproval(row) {
                ...
                $this.$confirm('是否确定提交审批?', '提示', {
                    ...
                    startProcessAsync(id,'FWTGHTZX').then((message)=>{
            ...
            editContractExecution(row){
                let url=this.$route.fullPath;
                this.$router.push({path:url+"/edit",query:{id:row.id,type:2}});
            },

            showVersionList(row){
                this.$refs.versionList.open(row.serviceHostingContractId);
            },
            ...
            exportData:function () {
                window.location = this.exportUrl("core/serviceHostingContractExecution/export?" + $.param(this.searchParam));
            },
            resetList(){
                this.organization=[];
                this.searchParam={};
