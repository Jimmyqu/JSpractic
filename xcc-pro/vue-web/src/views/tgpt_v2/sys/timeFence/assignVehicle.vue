                                    ...
                                    <li v-for="(va,i) in scope.row.vas" :key="i">
                                        <el-row :gutter="20" class="t_popover">
                                            ...
                                            <el-col :span="8">
                                                ...
                                                <el-tag size="small" type="info" v-if="va.fenceStatus==0">未启用</el-tag>
                                            </el-col>
                                        </el-row>

                                        <div class="city_wrap">允许运行时段: {{va.startTime}}-{{va.endTime}}</div>
                                        <div class="city_wrap">重复周期: {{va.cycleText}}</div>
                                    </li>
                                </ul>
    ...
    export default {
        ...
        methods:{
            ...
            open(id,organizationId){
                ...
                this.timeFenceId = id;
                this.getListByUrl("base//baseTimeFence/selectAddVehicle?timeFenceId=" + id+"&organizationId="+organizationId);
                this.show = true;
            },
            dblClickHandle() {
                let that = this;
                ...
                this.$confirm('确定添加（' + length + '）辆车?')
                    .then(() => {
                        ajax.post('/base/baseTimeFence/addVehicle', {areaId: this.timeFenceId, vehicleIds: vehicleIds})
                            .then(res => {
                                if(res.status==0) {
