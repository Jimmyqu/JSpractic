    ...
    export default {
        ...
        data() {
            return {
                ...
                listUrl: 'base//baseTimeFence/selectVehicle?timeFenceId=' + this.$route.params.id,
                timeFenceId: this.$route.params.id,
                dialog: {
                    dialogVehicle: false
                }
            }
        ...
        methods: {
            close(){
                this.$router.push({path:"/tgpt_v2/sys/timeFence"})
            },
            openAdd(){
                this.$refs.assignVehicle.open(this.timeFenceId,this.detailForm.companyId);
            },
            deleteVehicle(row){
                if(row){
                    this.$confirm('确定删除该车辆?', '提示', {
                        ...
                        var param = {
                            id: row.id,
                            timeFenceId: row.timeFenceId
                        };
                        ajax.post('base//baseTimeFence/deleteVehicle', param).then(rs => {
                            if (rs && rs.data == 1) {
                                this.$message({
                                    type: 'success',
                                    message: '删除成功!'
        ...
        mounted() {
            ajax.get('base//baseTimeFence/' + this.timeFenceId).then(rs => {
