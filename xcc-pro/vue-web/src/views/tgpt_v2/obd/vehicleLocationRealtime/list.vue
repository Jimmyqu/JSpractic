<template>
    <div class="app-container white-bg list-panel position" v-cloak>
            ...
            <!-- <ve-bmap :settings="mapSetting" :after-set-option-once="initMap" height="100%"
                style="width: 100%;height: calc(100vh - 120px)"></ve-bmap> -->
        ...
        <div class="frame" :class="{'hidden':shrink}">
            ...
            <h4 v-if="vehicleList && vehicleList.length">车辆:{{vehicleList.length}}辆</h4>
            <ul class="list-box" v-if="vehicleList && vehicleList.length">
                <li v-for="(bean,i) in vehicleList" @click="clickVehicleTrack(bean)" :key="i" style="cursor:pointer;padding-left: 10px" :class="{'active':bean.vehicleId == vehicleId}">
                    <p>
                        ...
                        <span class="title">{{bean.plate}}</span>
                    </p>
                    <p>
            ...
            <div class="shrink" v-if="vehicleList && vehicleList.length">
        ...
        methods: {
            initMap: function (echarts) {
               ...
               /* var geolocation = new BMap.Geolocation();
                ...
                // this.map.showScaleControl(false); // 标尺
            ...
            getVehicleExerciseInfo(vehicleId) {
                const loading = this.$loading({fullscreen: false, lock: true, background: 'rgba(255, 255, 255, 0.5)', target: document.querySelector('#map') });
                setTimeout(() => {
                    loading.close();
                }, 10000);
