        ...
        <el-form :model="ruleForm" :rules="rules" label-position="top" ref="ruleForm" label-width="100px">
            <el-collapse v-model="activeNames">
                ...
                <el-collapse-item title="合同基本信息" name="1">
                    <div class="flex-panel">
                        ...
                        <el-form-item label="客户类型" prop="customerType" >
                            <el-select v-model="ruleForm.customerType" @change="changeCustomerType" disabled :rules="rules.required('请选择客户类型')" placeholder="请选择客户类型">
                        ...
                        <el-form-item label="服务客户" prop="enterpriseId" :rules="rules.required('请选择服务客户')">
                            <el-input v-model="companyData.name"  disabled placeholder="请选择">
                                <!--@click.native="openEnterpriseModel()"-->
                                <el-button slot="append" icon="el-icon-search" disabled></el-button>
                            </el-input>
                        </el-form-item>
                        <el-form-item label="项目评审" prop="projectReviewId" >
                            <el-input v-model="projectData.projectName" placeholder="请选择" disabled>
                               <!-- clearable @click.native="openProjectModel()"-->
                                <el-button slot="append" icon="el-icon-search" disabled></el-button>
                            </el-input>
                        </el-form-item>
                ...
                <el-collapse-item title="合同明细" name="5">
                    ...
                    <el-table class="left" border :data="ruleForm.projectContractExecuteDetailList" style="width: 100%">
                        <el-table-column type="index" label="序号" width="50"></el-table-column>

                        <el-table-column label="服务组织" min-width="150" label-class-name="required">
                            <template slot-scope="{row,$index}" >
                                <el-form-item  :prop="'projectContractExecuteDetailList.' + $index + '.serviceOrganizationId'"
                                               :rules="rules.required('请选择服务组织')">
                                    <tree-select v-model="row.serviceOrganizationId" placeholder="请选择组织" type="one" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)"
                        ...
                        <el-table-column label="结算组织" min-width="150" label-class-name="required">
                            <template slot-scope="{row,$index}" >
                                <el-form-item  :prop="'projectContractExecuteDetailList.' + $index + '.settlementOrganizationId'"
                                               :rules="rules.required('请选择结算组织')">
                                    <tree-select v-model="row.settlementOrganizationId" placeholder="请选择组织" type="one" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)"
                                                 url="admin/organization/companyTree"></tree-select>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        <el-table-column prop="useCarCityName" show-overflow-tooltip label="用车城市" min-width="150">
                            <template slot-scope="{row,$index}" >
                                <el-form-item prop="city">
                                    <city-select :value.sync="row.city" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" :ref="'citySelect_' + $index">
                                    </city-select>
                                </el-form-item>
                        ...
                        <el-table-column prop="vehicleModelInfoName" show-overflow-tooltip label="车型" min-width="150" label-class-name="required">
                            <template slot-scope="{row,$index}" >
                                <el-form-item :prop="'projectContractExecuteDetailList.' + $index + '.dataModel'" :rules="rules.required('请选择车型')">
                                    <el-input type="text"  placeholder="请选择车型" v-model="row.dataModel"
                                              size="small" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)"
                                              @click.native="openDialogVehicleModelInfo($index,row.detailType)">
                                    </el-input>
                                </el-form-item>
                        ...
                        <el-table-column prop="useCarStartDate" label="用车开始时间" min-width="200" label-class-name="required">
                            <template slot-scope="{row,$index}">
                                <el-form-item :prop="'projectContractExecuteDetailList.' + $index + '.useCarStartDate'"  :rules="rules.required('请选择用车开始时间')">
                                    <el-date-picker type="date" placeholder="请选择" v-model="row.useCarStartDate"
                                                    value-format="yyyy-MM-dd" :picker-options="row.useCarStartDate" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)"
                                                    :editable="false"></el-date-picker>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        <el-table-column prop="useCarEndDate" label="用车结束时间" min-width="200" label-class-name="required">
                            <template slot-scope="{row,$index}">
                                <el-form-item :prop="'projectContractExecuteDetailList.' + $index + '.useCarEndDate'"  :rules="rules.required('请选择用车结束时间')">
                                <el-date-picker type="date" placeholder="请选择" v-model="row.useCarEndDate"
                                                value-format="yyyy-MM-dd" :picker-options="row.useCarEndDate" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)"
                                                :editable="false"></el-date-picker>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        ...
                        <el-table-column prop="carColorName" label="颜色" min-width="150">
                            <template slot-scope="{row,$index}" >
                                <el-form-item prop="carColor">
                                    <el-select v-model="row.carColor" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请选择">
                        ...
                        <el-table-column prop="carAge" label="车龄（月）" min-width="150">
                            <template slot-scope="{row,$index}" >
                                <el-form-item :prop="'projectContractExecuteDetailList.' + $index  + '.carAge'"
                                              :rules="{validator: numberThree, trigger: 'change'}">
                                    <el-input type="text" v-model="row.carAge" size="small" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)"
                        ...
                        <el-table-column prop="newOrOldCarName" label="车辆新旧" min-width="150" label-class-name="required">
                            <template slot-scope="{row,$index}" >
                                <el-form-item :prop="'projectContractExecuteDetailList.' + $index + '.newOrOldCar'" :rules="rules.required('请选择车辆新旧')">
                                    <el-select v-model="row.newOrOldCar" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请选择">
                        ...
                        <el-table-column label="每月车辆租金(元)" min-width="150" label-class-name="required">
                            <template slot-scope="{row,$index}" >
                                <el-form-item  :prop="'projectContractExecuteDetailList.' + $index + '.carRentMonth'"
                                               :rules="rules.money(true , '请输入车辆每月租金')">
                                    <el-input v-model="row.carRentMonth" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请输入"></el-input>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        <el-table-column label="测算租金(元)" min-width="150">
                            <template slot-scope="{row,$index}">
                                <el-form-item :prop="'projectContractExecuteDetailList.' + $index + '.calculatePrice'"  :rules="rules.money(false)">
                                    <el-input v-model="row.calculatePrice" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请输入"></el-input>
                                </el-form-item>
                        ...
                        <el-table-column prop="vehicleQty" label="车辆台数（台）" min-width="150" label-class-name="required">
                            <template slot-scope="{row,$index}">
                                <el-form-item :prop="'projectContractExecuteDetailList.' + $index + '.vehicleQty'" :rules="[{ validator: formRule.standardSize,message: '请输入整数', trigger: 'change' },{required: true, message: '请输入车辆台数', trigger: 'change'}]">
                                    <el-input v-model="row.vehicleQty" @blur="changeDriverCount(row)" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请输入"></el-input>
                                </el-form-item>
                        ...
                        <el-table-column prop="oilFeePaymenModel" label="油费支付类型" min-width="150" label-class-name="required">
                            <template slot-scope="{row,$index}">
                                <el-form-item :prop="'projectContractExecuteDetailList.' + $index  + '.oilFeePaymenModel'"
                                              :rules="{required: true, message: '请选择油费支付类型', trigger: 'change'}"  >
                                    <el-select v-model="row.oilFeePaymenModel" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请选择">
                        ...
                        <el-table-column label="每月车辆油费(元)" min-width="150">
                            <template slot-scope="{row,$index}">
                                <el-form-item :prop="'projectContractExecuteDetailList.' + $index + '.carOilCostMonth'"
                                              :rules="rules.money(false)" >
                                    <el-input v-model.number="row.carOilCostMonth" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请输入"></el-input>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        <el-table-column prop="date" label="每月限制里程公里" min-width="150">
                            <template slot-scope="{row,$index}">
                                <el-form-item :prop="'projectContractExecuteDetailList.' + $index + '.limitMileageMonth'"
                                              :rules="rules.int(false)">
                                    <el-input v-model="row.limitMileageMonth" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请输入" maxlength="10"></el-input>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        <el-table-column label="超里程单价(元)" min-width="150">
                            <template slot-scope="{row,$index}">
                                <el-form-item :prop="'projectContractExecuteDetailList.' + $index + '.overMileagePrice'"
                                              :rules="rules.threeDecimalMoney(false)">
                                    <el-input v-model="row.overMileagePrice" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请输入"></el-input>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        <el-table-column label="ETC费(元)" min-width="200">
                            <template slot-scope="{row,$index}">
                                <el-form-item :prop="'projectContractExecuteDetailList.' + $index + '.etcCost'"
                                              :rules="rules.money(false)">
                                    <el-input v-model="row.etcCost" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请输入"></el-input>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        <el-table-column prop="retrofitRequirement" label="加装要求" min-width="200">
                            <template slot-scope="{row,$index}">
                                <el-form-item :prop="'projectContractExecuteDetailList.' + $index + '.retrofitRequirement'">
                                    <el-input v-model="row.retrofitRequirement" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请输入"></el-input>
                                </el-form-item>
                        ...
                        <el-table-column prop="procurementExplanation" label="采购说明" min-width="200">
                            <template slot-scope="{row,$index}">
                                <el-form-item :prop="'projectContractExecuteDetailList.' + $index + '.procurementExplanation'">
                                    <el-input v-model="row.procurementExplanation" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请输入"></el-input>
                                </el-form-item>
                        ...
                        <el-table-column prop="retrofitRequirementOptionsText" label="加装要求选项" min-width="150">
                            <template slot-scope="{row,$index}">
                                <el-form-item>
                                    <el-select v-model="row.retrofitRequirementOptionsList" multiple :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)"
                                               placeholder="请选择">
                                        <el-option
                        ...
                        <el-table-column label="是否需要司机" min-width="150">
                            <template slot-scope="{row,$index}">
                                <el-form-item >
                                    <el-checkbox v-model="row.isNeedDriverBoolean" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" @click.native="changeNeedDriver(row)">是</el-checkbox>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        <el-table-column prop="date" label="司机每天工作时长(小时)" min-width="170">
                            <template slot-scope="{row,$index}">
                                <el-form-item v-if="row.isNeedDriverBoolean" :prop="'projectContractExecuteDetailList.' + $index + '.driverWorktimeDay'"
                                              :rules="rules.int(false,[1,24],'请输入1-24的整数')">
                                    <el-input v-model="row.driverWorktimeDay" placeholder="请输入" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" ></el-input>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        <el-table-column prop="date" label="司机数（名）" min-width="150" >
                            <template slot-scope="{row,$index}">
                                <el-form-item v-if="row.isNeedDriverBoolean" :prop="'projectContractExecuteDetailList.' + $index + '.driverCount'"
                                              >
                        ...
                        <el-table-column prop="date" label="每月司机费用(元)" min-width="150" label-class-name="required">
                            <template slot-scope="{row,$index}">
                                <el-form-item v-if="row.isNeedDriverBoolean" :prop="'projectContractExecuteDetailList.' + $index + '.driverCostMonth'"
                                              :rules="rules.money(true)">
                                    <el-input v-model="row.driverCostMonth" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请输入"></el-input>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        <el-table-column prop="date" label="每月司机通讯费(元)" min-width="150">
                            <template slot-scope="{row,$index}">
                                <el-form-item v-if="row.isNeedDriverBoolean" :prop="'projectContractExecuteDetailList.' + $index + '.driverCommunicationCostMonth'"
                                              :rules="rules.money(false)">
                                    <el-input v-model="row.driverCommunicationCostMonth" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请输入"></el-input>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        <el-table-column prop="date" label="代驾费(元)" min-width="150">
                            <template slot-scope="{row,$index}">
                                <el-form-item v-if="row.isNeedDriverBoolean" :prop="'projectContractExecuteDetailList.' + $index + '.drivingCost'"
                                              :rules="rules.money(false)">
                                    <el-input v-model="row.drivingCost" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请输入"></el-input>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        <el-table-column prop="date" label="加班费（小时）" min-width="150">
                            <template slot-scope="{row,$index}">
                                <el-form-item v-if="row.isNeedDriverBoolean" :prop="'projectContractExecuteDetailList.' + $index + '.overtimeCost'"
                                              :rules="rules.money(false)">
                                    <el-input v-model="row.overtimeCost" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请输入"></el-input>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        <el-table-column prop="date" label="每晚住宿费(元)" min-width="150">
                            <template slot-scope="{row,$index}">
                                <el-form-item v-if="row.isNeedDriverBoolean" :prop="'projectContractExecuteDetailList.' + $index + '.stayCost'"
                                              :rules="rules.money(false)">
                                    <el-input v-model="row.stayCost" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请输入"></el-input>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        <el-table-column prop="date" label="餐费（次）" min-width="150">
                            <template slot-scope="{row,$index}">
                                <el-form-item v-if="row.isNeedDriverBoolean":prop="'projectContractExecuteDetailList.' + $index + '.mealCost'"
                                              :rules="rules.money(false)">
                                    <el-input v-model="row.mealCost" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请输入"></el-input>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        <el-table-column prop="date" label="福利费(元)" min-width="150">
                            <template slot-scope="{row,$index}">
                                <el-form-item v-if="row.isNeedDriverBoolean" :prop="'projectContractExecuteDetailList.' + $index + '.welfareFee'"
                                              :rules="rules.money(false)">
                                    <el-input v-model="row.welfareFee" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请输入"></el-input>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        <el-table-column prop="date" label="年高温补贴费(元)" min-width="150">
                            <template slot-scope="{row,$index}">
                                <el-form-item v-if="row.isNeedDriverBoolean" :prop="'projectContractExecuteDetailList.' + $index + '.highTemperatureFee'"
                                              :rules="rules.money(false)">
                                    <el-input v-model="row.highTemperatureFee" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请输入"></el-input>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        <el-table-column prop="date" label="年生日补贴费(元)" min-width="150">
                            <template slot-scope="{row,$index}">
                                <el-form-item v-if="row.isNeedDriverBoolean" :prop="'projectContractExecuteDetailList.' + $index + '.birthdayFee'"
                                              :rules="rules.money(false)">
                                    <el-input v-model="row.birthdayFee" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请输入"></el-input>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        <el-table-column prop="date" label="其他补贴费(元)" min-width="150">
                            <template slot-scope="{row,$index}">
                                <el-form-item v-if="row.isNeedDriverBoolean" :prop="'projectContractExecuteDetailList.' + $index + '.otherSubsidyCost'"
                                              :rules="rules.money(false)">
                                    <el-input  v-model="row.otherSubsidyCost" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请输入"></el-input>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        <el-table-column prop="date" label="其他费用说明" min-width="150">
                            <template slot-scope="{row}">
                                <el-form-item v-if="row.isNeedDriverBoolean" >
                                    <el-input v-model="row.otherCostExplanation" :disabled="(ruleForm.version==1 && row.id!=null) || (ruleForm.version!=1 && row.previousVersionId!=null)" placeholder="请输入" maxlength="50"></el-input>
                                </el-form-item>
                            </template>
                        </el-table-column>
    ...
    export default {
        ...
        data() {
            ...
            return {
                ...
                saveDisabled:false,
                editFlag: false,//执行时明细可编辑，编辑时明细不可编辑
                editable:"",
        ...
        methods: {
            ...
            getCountSum(){
                let sum = 0;
                let data = this.ruleForm.projectContractExecuteDetailList;
                if(data && data.length) {
                    data.forEach( item =>{
            ...
            setInitData(){
                ...
                this.ruleForm = {
                    ...
                    projectContractExecuteDetailList:[
                        {
                            cityName: [],
                            dataModel: '',
                            vehicleModelInfoId:"",
            ...
            saveProject(row){
                ajax.get("/core/coreProjectReview/detailByInquiry", {id:row.id}).then(res => {
                    if (this.checkResponse(res)) {// 验证请求成功状态
                        ...
                        this.ruleForm.projectContractExecuteDetailList = res.data.inquiry;
                        console.log(this.ruleForm.projectContractExecuteDetailList )
            ...
            invoiceTaxRateVehicleChange(){
                ...
                if(this.ruleForm.projectContractExecuteDetailList){
                    this.ruleForm.projectContractExecuteDetailList.forEach(row=>{
            ...
            open(){
                ...
                this.editable = this.$route.query.editable;//编辑编辑中的合同执行(审批不通过的)
                if(this.type==2){
                    this.editFlag=true;
                }
                this.setInitData();
            ...
            getFormData: function (id) {
                this.fileList = [];
                this.confirmType=[];
                ajax.get("core/coreProjectContractExecute/getDetail?id=" + id).then(result =>{
                    if (result.status == 0) {
                        if( result.data[0]){
                            ...
                            if(result.data[0].attachment){
                                this.fileList.push(JSON.parse(result.data[0].attachment));
                            }
                        ...
                        if( result.data[1]){
                            ...
                            this.ruleForm.projectContractExecuteDetailList =  result.data[1];
                        }
            ...
            addProjectContractDetail() {
                this.ruleForm.projectContractExecuteDetailList.push({
            ...
            save() {
                this.$refs.ruleForm.validate((valid) => {
                    if (valid) {
                        let detailList=this.ruleForm.projectContractExecuteDetailList;
                        ...
                        let url="";
                        if(this.type==1){
                            url="/core/coreProjectContractExecute/perform";
                        }else if(this.type==2){
                            url="/core/coreProjectContractExecute/edit";
                            if(this.editable == 1)
                                url="/core/coreProjectContractExecute/reedit";
                        }
            ...
            convertData(params){
                ...
                params.projectContractExecuteDetailList.forEach( item =>{
            ...
            openDialogVehicleModelInfo(i,detailType){
                if(this.editFlag&&!detailType){
            ...
            selectVehModelInfo(cell) {
                ...
                this.$set(this.ruleForm.projectContractExecuteDetailList[i],"dataModel",cell.modelInfo);
                this.$set(this.ruleForm.projectContractExecuteDetailList[i],"vehicleModelInfoId",cell.id);
                /*  this.ruleForm.projectContractExecuteDetailList[i].dataModel = cell.modelInfo
                  this.ruleForm.projectContractExecuteDetailList[i].vehicleModelInfoId = cell.id;*/
                this.dialog.vehInfo = false;
            },
            deleteItem(index,row) {
                if (this.ruleForm.projectContractExecuteDetailList.length == 1) {
                    this.$message.error('至少保留一条合同执行明细');
                    return;
                }

                if(this.type == 2){//处于合同执行编辑模式
                    if((this.ruleForm.version==1 && row.id) || (this.ruleForm.version!=1 && row.previousVersionId)){
                        this.$message.error('编辑合同执行是不允许删除合同执行明细');
                        return;
                    }
                }
                this.ruleForm.projectContractExecuteDetailList.splice(index, 1);
            }
        },
