...
export default props => {
  ...
  const fetching = useSelector(state => state.loading.effects['pubserviceuser/fetchCertUserCareerList']);

  useEffect(() => {
    dispatch({
      type: 'pubserviceuser/fetchCertUserCareerList',
    }).then(data => {
      setCareerList(
        (data || [])
          .map(item => {
  ...
  return (
    <AuthTabsPageHeaderLayout
      ...
      // title="人员列表"
      authScopePath="/basic/certmng/list"
      tabList={careerList}
    />
  );
};
