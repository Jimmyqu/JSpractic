    ...
    export default {
        ...
        data() {
            return {
                showSearch: false,
                searchParam: {},
                showEditBtn: this.getCurrentUserAuthority('baseElectron/edit'),
                showAddBtn: this.getCurrentUserAuthority('baseElectron/add'),
                showEnabledBtn: this.getCurrentUserAuthority('baseElectron/enable'),
                showExportExcelBtn: true,
                showFenceToVehicleBtn: this.getCurrentUserAuthority('baseElectron/areaFenceToVehicle'),
                showDetailBtn: this.getCurrentUserAuthority('baseElectron/detail'),
                showDeleteBtn: this.getCurrentUserAuthority('baseElectron/delete'),
                listUrl: '/base/baseElectron'
            }
        },
        activated() {
            this.getList();
        ...
        methods: {
            ...
            areaFenceToVehicle(id) {
                let url = this.$route.fullPath + '/electronFenceToVehicle?id=' + id;
                this.$router.push({path: url});
            },
            _delete(obj) {
                this.$confirm('确定要删除电子围栏【' + obj.name + '】?')
                    .then(() => {
                        ajax.delete('/base/baseElectron/' + obj.id).then(res => {
